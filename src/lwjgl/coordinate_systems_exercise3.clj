(ns lwjgl.coordinate-systems-exercise3
  (:gen-class)
...
(defn run-example!
  ...
  (let [width 800
        ...
        window (core/create-window width height "LearnOpenGL - Coordinate Systems Exercise 3 (LWJGL)")]
    (try
          ...
          (loop []
            (when-not (GLFW/glfwWindowShouldClose window)
              ...
              (doseq [[idx pos] (map-indexed vector cube-positions)]
                (.identity model)
                (.translate model pos)
                (.rotate model (+ (* 0.25 idx) (float (GLFW/glfwGetTime))) 0.5 1.0 0.2)
