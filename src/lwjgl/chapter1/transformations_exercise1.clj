(ns lwjgl.chapter1.transformations-exercise1
  (:gen-class)
...
(defn- create-texture
  ...
  (let [tex (GL45/glGenTextures)
    ...
    (dotimes [y h]
      (dotimes [x w]
        (let [checker (if (zero? (bit-and (+ (quot x 16) (quot y 16)) 1)) 255 70)
              idx (* 3 (+ x (* y w)))]
          (.put buf idx (unchecked-byte checker))
          (.put buf (inc idx) (unchecked-byte checker))
          (.put buf (+ idx 2) (unchecked-byte 180)))))
    (.rewind buf)
...
(defn run-example!
  ...
  (let [width 800
        ...
        window (u/create-window width height "LearnOpenGL - Transformations Exercise 1 (LWJGL)")]
    (try
      ...
      (let [program (u/create-program vertex-shader-source fragment-shader-source)
        ...
        (try
          ...
          (when (<= 0 tex-loc) (GL45/glUniform1i tex-loc 0))
          (loop []
            (when-not (GLFW/glfwWindowShouldClose window)
              (let [time (float (GLFW/glfwGetTime))
                    first (doto (Matrix4f.)
                            ...
                            (.scale (float (+ 0.5 (* 0.5 (Math/abs (Math/sin time)))))))
                    second (doto (Matrix4f.)
                             (.identity)
                             (.translate 0.5 -0.5 0.0)
                             (.rotate (* -1.0 time) 0.0 0.0 1.0)
                             (.scale 0.5))]
                ...
                (GL45/glBindTexture GL45/GL_TEXTURE_2D tex)
                (GL45/glBindVertexArray vao)
                (upload-transform! first buf transform-loc)
                (GL45/glDrawElements GL45/GL_TRIANGLES 6 GL45/GL_UNSIGNED_INT 0)
                (upload-transform! second buf transform-loc)
                (GL45/glDrawElements GL45/GL_TRIANGLES 6 GL45/GL_UNSIGNED_INT 0)
