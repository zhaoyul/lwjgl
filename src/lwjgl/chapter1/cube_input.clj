  ...
  (let [width (atom 900)
    ...
    (try
      ...
      (let [program (u/create-program vertex-shader-source fragment-shader-source)
            ...
            model (Matrix4f.)
            view (doto (Matrix4f.) (.translate 0.0 0.0 -3.5))
            projection (Matrix4f.)]
        (try
          ...
          (GLFW/glfwSetScrollCallback
           ...
           (reify GLFWScrollCallbackI
             (invoke [_ _ _ yoffset]
               (swap! scale #(clamp (+ % (* 0.08 yoffset)) 0.4 2.2)))))
          ...
          (loop []
            (when-not (GLFW/glfwWindowShouldClose window)
              (let [now (GLFW/glfwGetTime)
                ...
                (reset! scale (float (clamp new-scale 0.4 2.2)))
                ...
                (GL45/glClearColor 0.05 0.06 0.08 1.0)
