(ns lwjgl.chapter1.coordinate-systems-exercise3
  (:gen-class)
...
(defn- create-texture
  ...
  (let [tex (GL45/glGenTextures)
    ...
    (dotimes [y h]
      (dotimes [x w]
        (let [checker (if (zero? (bit-and (+ (quot x 16) (quot y 16)) 1)) 255 140)
              idx (* 3 (+ x (* y w)))]
          (.put buf idx (unchecked-byte checker))
          (.put buf (inc idx) (unchecked-byte checker))
          (.put buf (+ idx 2) (unchecked-byte 190)))))
    (.rewind buf)
...
(defn run-example!
  ...
  (let [width 800
        ...
        window (u/create-window width height "LearnOpenGL - Coordinate Systems Exercise 3 (LWJGL)")]
    (try
          ...
          (loop []
            (when-not (GLFW/glfwWindowShouldClose window)
              (let [time (float (GLFW/glfwGetTime))
                    radius 6.0
                    cam-x (* radius (Math/sin time))
                    cam-z (* radius (Math/cos time))]
                (.identity view)
                (.lookAt view (Vector3f. (float cam-x) 0.0 (float cam-z))
                         (Vector3f. 0.0 0.0 0.0)
                         (Vector3f. 0.0 1.0 0.0))
                (upload-mat! view mat-buf view-loc))
              (GL45/glClearColor 0.08 0.1 0.12 1.0)
