(ns lwjgl.chapter1.shaders-exercise1
  (:gen-class)
...
out vec3 ourColor;
void main() {
    gl_Position = vec4(aPos.x, -aPos.y, aPos.z, 1.0); // `负值`y
    ourColor = aColor;
}")
...
(defn run-example!
  ...
  (let [width 800
        ...
        window (u/create-window width height "LearnOpenGL - Shaders Exercise 1 (LWJGL)")]
    (try
