...
(defn run-example!
  ...
  (let [width 800
        ...
        window (u/create-window width height "LearnOpenGL - Textures (LWJGL)")]
    (try
      ...
      (let [program (u/create-program vertex-shader-source fragment-shader-source)
            ...
            sampler-loc (GL45/glGetUniformLocation program "texture1")]
        (try
          ...
          (when (<= 0 sampler-loc)
            ...
            (GL45/glUniform1i sampler-loc 0))
          (loop []
          ...
          (finally
            ...
            (delete-if-positive texture #(GL45/glDeleteTextures %)))))
      (finally
