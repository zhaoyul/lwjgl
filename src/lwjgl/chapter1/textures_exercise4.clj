...
(defn- create-border-texture
  ...
  (let [tex (GL45/glGenTextures)
    ...
    (GL45/glTexParameterfv GL45/GL_TEXTURE_2D GL45/GL_TEXTURE_BORDER_COLOR border-color)
    (GL45/glTexImage2D GL45/GL_TEXTURE_2D 0 GL45/GL_RGB w h 0 GL45/GL_RGB GL45/GL_UNSIGNED_BYTE buf)
    tex))
...
(defn run-example!
  ...
  (let [width 800
        ...
        window (u/create-window width height "LearnOpenGL - Textures Exercise 4 (LWJGL)")]
    (try
